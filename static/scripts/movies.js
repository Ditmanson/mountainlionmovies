...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Function to fetch and display watchlist films
  const fetchWatchlist = () => {
    fetch(`/api/watchlist/`)
      .then(response => response.json())
      .then(data => {
        if (data.length > 0) {
          populateCarousel(data, 'watchlist');
        }
      })
      .catch(error => console.error('Error fetching watchlist:', error));
  };

  // Function to fetch and display seen films
  const fetchSeenFilms = () => {
    fetch(`/api/seen_films/`)
      .then(response => response.json())
      .then(data => {
        if (data.length > 0) {
          populateCarousel(data, 'seenFilms');
        }
      })
      .catch(error => console.error('Error fetching seen films:', error));
  };

  // Function to populate a carousel with fetched data
  const populateCarousel = (films, elementId) => {
    ...
    if (films.length > 0) {
      films.forEach((film, index) => {
        const isActive = index === 0 ? 'active' : ''; // Mark the first item as active
        const movieElement = document.createElement('div');
        movieElement.classList.add('carousel-item', isActive);
        movieElement.innerHTML = `
          <img src="${baseUrl}${film.film.poster_path}" class="d-block w-100" alt="${film.film.title}">
          <div class="carousel-caption d-none d-md-block">
            <h5>${film.film.title}</h5>
            <p>${film.film.overview}</p>
            <p>Rating: ${film.film.vote_average}</p>
          </div>
        `;

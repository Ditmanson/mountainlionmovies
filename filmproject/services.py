...
from filmproject.models import LT_Viewer_Seen, LT_Viewer_Cosine_Similarity, Viewer

@transaction.atomic
def calculate_cosine_similarity():
    # Step 1: Fetch viewer ratings
    ratings = LT_Viewer_Seen.objects.filter(viewer_rating__isnull=False).values(
        'viewer_id', 'film_id', 'viewer_rating'
    )
    
    # Step 2: Construct User-Movie matrix
    df = pd.DataFrame(ratings)
    user_movie_matrix = df.pivot_table(
        index='viewer_id', columns='film_id', values='viewer_rating', fill_value=0
    )
